...
func StructNonZeroValueFieldToMap(obj interface{}) (map[string]interface{}, error) {
	objValue := reflect.ValueOf(obj)
	...
	for i := 0; i < objValue.NumField(); i++ {
		field := objValue.Field(i)
		isZeroValue := reflect.DeepEqual(field.Interface(), reflect.Zero(field.Type()).Interface())
		if isZeroValue {
			continue
		}
		...
		// 分割json标签，通常格式为"name,"
		jsonNames := strings.Split(jsonTag, ",")
		if len(jsonNames) > 0 {
			// 使用第一个名称作为映射的键
			fieldName := jsonNames[0]
			result[fieldName] = field.Interface()
		}
	}
